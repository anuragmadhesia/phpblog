...
<div class="container-fluid p-4">
    <div class="row">
        <div class="col-sm-9">
        <h4>Articles in tag:- <?php echo htmlspecialchars($_GET['id']);?></h4>
        <hr>
        <?php
            try {
                $stmt = $db->prepare('SELECT articleId, articleTitle, articleSlug, articleDescription, articleDate, articleTags FROM blog WHERE articleTags like :articleTags ORDER BY articleId DESC');
                ...
                while($row = $stmt->fetch()){
                    echo '<div class="card my-3"><div class="card-header">';
                        ...
                        if ($row['articleTags']!=null) {
                            ...
                            {
                                $links[] = "<a href='".$tags."' class='btn btn-secondary btn-sm py-0 px-1 mr-2'>".$tags."</a>";
                            }
                            echo implode("", $links);
                            echo '</p>';
                        }
